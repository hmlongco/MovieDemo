...
2. Minimum code that solves the problem. Nothing speculative unless asked.
...
## Core instructions
- Target iOS 17.0 or later. (Yes, it definitely exists.)
...
If installed, use Factory for dependency injection of services and repositories.
```swift
import FactoryKit

extension Container {
    package var orderNetworking: Factory<OrderNetworking> {
        self { self.apiClient() }
            #if DEBUG
            .onPreview { MockOrderNetworking() }
            #endif
    }
}
```
Inject dependencies using `@Injected`:
```swift
@Injected(\.orderNetworking) private var orderNetworking
```

### Navigation (Navigator)
If installed, use Navigator and its destination enums for SwiftUI view navigation and presentation.
**Destination enum pattern:**
import NavigatorUI

```swift
enum HomeDestinations {
    case productCarousel(selectedProductId: String, allProducts: [AnyProductDisplaying], event: Event)
    case upcomingEvents
    case referral
}
...
extension HomeDestinations: NavigationDestination {
    var body: some View {
        switch self {
        case let .productCarousel(selectedProductId, allProducts, event):
            CarouselView(mode: .products(allProducts.map(\.wrapped), selectedId: selectedProductId),
                         configuration: .init(event: event))
        case .upcomingEvents:
            UpcomingView()
        case .referral:
            ReferralMainView(source: .home)
        }
    }

    var method: NavigationMethod {
        switch self {
        case .productCarousel, .referral: .managedSheet
        case .upcomingEvents: .push
        }
    }
}
```

**Triggering navigation from ViewModels:**
```swift
func didTapEvent(event: Event, navigator: Navigator) {
    navigator.navigate(to: HomeDestinations.upcomingEvents)
}
```
**Navigation methods:** `.push`, `.managedSheet`, `.sheet`, `.cover`
